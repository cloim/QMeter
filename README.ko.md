...
- Codex 사용량 조회에는 Codex CLI ChatGPT 로그인 필요

## 빌드/실행

release 바이너리 이름은 CLI가 `qmeter.exe`, 트레이 앱이 `qmeter-tray.exe`입니다.
CLI Cargo package도 `qmeter`라서 소스에서 개발 실행할 때는 `-p qmeter`를 사용합니다.
...
Codex 사용량은 Codex CLI가 저장한 `~/.codex/auth.json`의 ChatGPT OAuth token으로 조회합니다. QMeter가 Codex usage endpoint를 직접 호출하므로 새로고침 시 `codex.cmd`나 `cmd.exe`를 실행하지 않습니다.
...
- Codex row가 없으면 Codex CLI ChatGPT 로그인 또는 `~/.codex/auth.json` 상태를 확인하세요.
